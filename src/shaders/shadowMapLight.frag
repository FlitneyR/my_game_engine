...
layout(set = 1, binding = 1) uniform Uniform_1_0 { Camera lightView; };
...
float pcfFilter(vec3 worldPos, float filterSize, int filterSteps);
...
void main() {
    ...
    float shadow = pcfFilter(gd.worldPos, 2, 5);
    if (shadow <= 0) return;
